...
def _fetch_theoddsapi(day):
    ...
    start = day.replace(hour=15, minute=0).isoformat() + "Z"
    end = (day + timedelta(days=2)).replace(hour=10, minute=0).isoformat() + "Z"
    params = {
        "apiKey": api_key,
        "regions": "us,eu,us2",
    ...
    def _get():
        for attempt in range(4):
            ...
            if r.status_code != 200:
                raise ApiError(f"The Odds API {r.status_code}: {r.text[:200]}")
            return r.json()
        raise ApiError("The Odds API 429 after retries")
    data = _get()
    print(f"[data] Using The Odds API — {len(data)} events in one call")
    remaining = requests.get(url, params=params, timeout=TIMEOUT).headers.get("x-requests-remaining","?")
...
def main():
    ...
    print(f"[slate] Fetched {len(fixtures)} pre-game fixtures")
    ...
    payload={"date":now.strftime("%Y-%m-%d"),"generated_at_utc":now.isoformat(),
             "source":{"provider":"The Odds API / OddsPapi","bookmaker":BOOKMAKER},
