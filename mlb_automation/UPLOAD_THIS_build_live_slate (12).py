...
# ── Rate-limit tuning ──
# We no longer call /markets (the hot endpoint), so pacing is much gentler.
INITIAL_COOLDOWN_SEC = float(os.getenv("ODDSPAPI_INITIAL_COOLDOWN", "3"))
BETWEEN_FIXTURE_DELAY_SEC = float(os.getenv("ODDSPAPI_FIXTURE_DELAY", "1.2"))
MAX_ODDS_RETRIES = int(os.getenv("ODDSPAPI_MAX_RETRIES", "10"))
...
def odds_get(endpoint: str, params: dict, max_retries: int = MAX_ODDS_RETRIES, use_cache: bool = False, cache_ttl: float = MARKET_CACHE_TTL_SEC) -> dict | list:
    ...
    - Reads from a simple file-based cache (keyed by endpoint+params) when use_cache=True.
    - The market catalog is effectively static within a day, so we cache it for 1 hour.
    - Backoff schedule tuned for ~3 minute cooldown capacity to survive aggressive windows.
    """
    api_key = env_required("ODDSPAPI_API_KEY")
    merged = {"apiKey": api_key, **params}
    url = f"{BASE_URL}/{endpoint.lstrip('/')}"

    # Cache key excludes apiKey
    ...
    waits = [3.0, 6.0, 10.0, 20.0, 30.0, 45.0, 60.0, 60.0, 60.0, 60.0, 60.0]
    for attempt in range(max_retries + 1):
        ...
        if response.status_code == 429:
            ...
            try:
                ...
                wait = None
            if wait is None:
                wait = waits[min(attempt, len(waits)-1)] + random.uniform(0.0, 1.0)
            if attempt < max_retries:
                print(f"[odds] 429 on {endpoint} (attempt {attempt+1}/{max_retries+1}), waiting {wait:.0f}s...")
                _time.sleep(wait)
                continue
            raise ApiError(f"429 Too Many Requests (gave up after {max_retries+1} tries over ~5min)") from None
        try:
            response.raise_for_status()
