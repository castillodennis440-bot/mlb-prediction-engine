...
def odds_get(endpoint: str, params: dict) -> dict | list:
    ...
    try:
        ...
        # Include Retry-After hint in error message when present
...
def main() -> None:
    ...
    for i, fixture in enumerate(fixtures):
        ...
        # Stagger base delay between fixtures to avoid burst 429s
        if i > 0:
            _time.sleep(1.2)
        book = None
        last_err = None
        for attempt in range(MAX_RETRIES + 1):
            try:
                ...
                if "429" in err_msg and attempt < MAX_RETRIES:
                    wait = BASE_DELAY * (2 ** attempt) + random.uniform(0.0, 0.5)
                    print(f"[slate] 429 on {p2_raw} @ {p1_raw} (attempt {attempt+1}), waiting {wait:.1f}s...")
                    _time.sleep(wait)
                    continue
                break
        if book is None:
            err_msg = sanitize_error(last_err) if last_err else "unknown error"
            skipped.append(
                {
