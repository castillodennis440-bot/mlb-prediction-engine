...
# Force UTF-8 for all HTTP responses (prevents mojibake like MartÃ­n)
...
def match_schedule_game(away_team, home_team, start_time, schedule_games):
    ...
    def _swap(s):
        ...
        return o
    def _ds(g):
        gt=_parse_iso(g.get("start_time")); return abs((gt-target).total_seconds()) if gt and target else 1e18
...
def build_game(fixture, book, schedule_games, fc, pc, now):
    ...
    # Market-heavy blend: 70% market-derived lambdas, 30% custom form/pitcher model.
    # This keeps the model honest — Pinnacle/Consensus is always closer to true
    # probability than our own form signals; custom signals add edge at the margin.
    lh9=round(0.70*mh9+0.30*mhome9,3); la9=round(0.70*ma9+0.30*maway9,3)
    lh5=round(0.70*mh5+0.30*mhome5,3); la5=round(0.70*ma5+0.30*maway5,3)
...
def main():
    ...
    fc={}; pc={}; games=[]; skipped=[]; seen_game_keys=set()
    for i,fx in enumerate(fixtures):
        fid=fx.get("fixtureId")
        if not fid: continue
        ...
        try:
            ...
            if gm:
                # Dedupe: same away@home@start-hour → same game (handles doubleheader
                # edge cases if both feeds match the wrong game).
                st=str(gm.get('source_meta',{}).get('start_time',''))[:13]
                gk = f"{gm['away_team']}|{gm['home_team']}|{st}"
                if gk not in seen_game_keys:
