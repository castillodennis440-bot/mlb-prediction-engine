...
# ── Rate-limit tuning ──
INITIAL_COOLDOWN_SEC = float(os.getenv("ODDSPAPI_INITIAL_COOLDOWN", "2"))
BETWEEN_FIXTURE_DELAY_SEC = float(os.getenv("ODDSPAPI_FIXTURE_DELAY", "0.8"))
MAX_ODDS_RETRIES = int(os.getenv("ODDSPAPI_MAX_RETRIES", "4"))
MAX_TOTAL_SLEEP_PER_CALL_SEC = float(os.getenv("ODDSPAPI_MAX_SLEEP_PER_CALL", "30"))
MARKET_CACHE_TTL_SEC = float(os.getenv("ODDSPAPI_MARKET_CACHE_TTL", "3600"))  # 1hr
...
# Directory for persistent cache (survives across re-runs on same runner)
CACHE_DIR = Path(os.getenv("ODDSPAPI_CACHE_DIR", "mlb_automation/.odds_cache"))
...
def _read_cache(key: str, ttl_sec: float):
    p = _cache_path(key)
    if not p.exists():
        return None
    try:
...
def odds_get(endpoint: str, params: dict, max_retries: int = MAX_ODDS_RETRIES, use_cache: bool = False, cache_ttl: float = MARKET_CACHE_TTL_SEC) -> dict | list:
    """Call OddsPapi with exponential-backoff retry on 429.

    Backoff is capped so a single call never sleeps more than MAX_TOTAL_SLEEP_PER_CALL_SEC
    seconds total (prevents multi-minute hangs during manual re-runs).
    """
    ...
    if use_cache:
        ...
        if cached is not None:
            print(f"[odds] Using cached response for {endpoint}")
            return cached

    waits = [2.0, 5.0, 10.0, 20.0, 30.0, 45.0]
    total_slept = 0.0
    for attempt in range(max_retries + 1):
        ...
        if response.status_code == 429:
            ...
            try:
                ...
                wait = None
            if wait is None:
                wait = waits[min(attempt, len(waits)-1)] + random.uniform(0.0, 0.5)
            if attempt < max_retries and (total_slept + wait) <= MAX_TOTAL_SLEEP_PER_CALL_SEC:
                print(f"[odds] 429 on {endpoint} (attempt {attempt+1}/{max_retries+1}), waiting {wait:.0f}s...")
                _time.sleep(wait)
                total_slept += wait
                continue
            raise ApiError(
                f"429 Too Many Requests (exceeded {MAX_TOTAL_SLEEP_PER_CALL_SEC:.0f}s retry budget "
                f"after {attempt+1} tries; total slept {total_slept:.0f}s)"
            ) from None
        try:
            response.raise_for_status()
        ...
        return data
    raise ApiError("odds_get failed")
...
def load_market_catalog() -> dict[int, dict]:
    """Kept for backwards compatibility but no longer needed — market name/handicap are read
    directly from per-fixture book odds responses. Returns an empty dict."""
    return {}
...
def market_info(book_markets: dict, market_id: str) -> tuple[str, float | None]:
    """Extract market name and handicap DIRECTLY from the bookmaker market entry
    (no separate /markets catalog call needed). Each OddsPapi market dict carries
    marketName and handicap inline."""
    try:
        mid = str(market_id)
    except Exception:
        return "", None
    m = (book_markets or {}).get(mid) or {}
    return str(m.get("marketName", "") or ""), try_float(m.get("handicap"))


def _identify_outcomes(market_entry: dict) -> dict:
    """Return a {outcomeId_str: outcomeName_lower} map from the market entry's outcomes."""
    outcomes = {}
    for out in (market_entry.get("outcomes") or []):
        try:
...
def choose_best_total(book_markets: dict, target_phrase: str) -> dict | None:
    candidates = []
    for mid, market in (book_markets or {}).items():
        name, handicap = market_info(book_markets, mid)
        if name != target_phrase:
            continue
        prices = [extract_price((onode.get("players") or {}).get("0")) for onode in (market.get("outcomes") or [])]
...
def choose_moneyline(book_markets: dict, target_phrase: str) -> dict | None:
    for mid, market in (book_markets or {}).items():
        name, _ = market_info(book_markets, mid)
        if name != target_phrase:
            continue
        prices = []
        for onode in (market.get("outcomes") or []):
            oid = onode.get("outcomeId")
...
def choose_handicap(book_markets: dict, target_phrase: str, wanted_line: float, home_ml: float, away_ml: float) -> dict | None:
    candidates = []
    for mid, market in (book_markets or {}).items():
        name, handicap = market_info(book_markets, mid)
...
def build_game(
    ...
    f5_ml = choose_moneyline(markets, "Winner 1st 5 innings")
    f5_total = choose_best_total(markets, "Over Under 1st 5 innings")
    f5_reference = f5_ml if f5_ml else fg_ml
    f5_rl = choose_handicap(markets, "Handicap 1st 5 innings", 0.5, f5_reference["home"], f5_reference["away"]) if f5_reference else None
...
def main() -> None:
    ...
    # Startup cool-down: if this run was triggered right after a failed 429 run, give OddsPapi's
    # per-minute window time to clear before we hit them again. (Won't fire on cron because cron
    # only runs twice a day with natural spacing.)
    cooldown = INITIAL_COOLDOWN_SEC
    print(f"[slate] Waiting {cooldown:.0f}s before first OddsPapi call to let rate limits settle...")
    _time.sleep(cooldown)
    # No longer calling /markets — market metadata is read from each fixture's odds payload.
    catalog = {}
    ...
    for i, fixture in enumerate(fixtures):
        ...
        if i > 0:
            _time.sleep(BETWEEN_FIXTURE_DELAY_SEC)
        try:
            book = fetch_book_odds(fixture_id)
            game = build_game(fixture, book, schedule_games, form_cache, pitcher_cache, now_utc)
            if game:
                games.append(game)
            else:
                skipped.append(
