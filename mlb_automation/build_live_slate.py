...
def _fetch_theoddsapi(day):
    ...
    # Start from NOW (script run time) so we don't lose games that already
    # commenced today — The Odds API drops in-play/completed events from the feed.
    # End at day+2 10:00 UTC to capture post-midnight West Coast games.
    ...
    # Valid regions are us, uk, eu, au (us2 is not valid). Pinnacle is in 'eu'.
    params = {
        "apiKey": api_key,
        "regions": "eu",
    ...
    resp = _get()
    data = resp.json()
    # If bookmakers=pinnacle returned empty (e.g. Pinnacle slow to post for a game),
    # retry without the bookmaker filter and pick Pinnacle from what comes back.
    if not data:
        print("[the-odds-api] Empty result with bookmakers=pinnacle; retrying without filter...")
        params2 = dict(params); params2.pop("bookmakers", None)
        for attempt in range(2):
            r2 = requests.get(url, params=params2, timeout=TIMEOUT)
            if r2.status_code == 200:
                raw = r2.json()
                # Keep only events that have Pinnacle in their bookmakers list
                filt = []
                for ev in raw:
                    bms = ev.get("bookmakers") or []
                    has_p = any((bm.get("key") == "pinnacle") for bm in bms)
                    if has_p:
                        ev["bookmakers"] = [bm for bm in bms if bm.get("key") == "pinnacle"]
                        filt.append(ev)
                print(f"[the-odds-api] Unfiltered call returned {len(raw)} events; {len(filt)} have Pinnacle")
                if filt:
                    data = filt
                    resp = r2
                break
    ...
    print(f"[data] Using The Odds API — {len(data)} events in one call (used {used}, remaining {remaining})")
    ...
    for ev in data:
        ...
        markets = {}
        for bm in bms:
            for mi, mk in enumerate(bm.get("markets", [])):
                key = mk.get("key")
                ...
                if key == "h2h":
                    outcomes = []
                    for oc in outs:
                        tn = normalize_team(oc.get("name"))
                        is_home = (_team_key(tn) == _team_key(home))
                        oid = oid_counter + (0 if is_home else 1)
                        outcomes.append({"outcomeId": oid, "outcomeName": tn.lower(),
                                         "players": {"0": {"price": oc.get("price")}}})
                    oid_counter += 2
                    markets[str(1000+mi)] = {"marketName":"Winner (incl. extra innings)","handicap":None,"outcomes":outcomes}
                elif key == "spreads":
                    pts = [oc.get("point") for oc in outs]
                    handicap = abs(pts[0]) if pts and pts[0] is not None else 1.5
                    outcomes = []
                    for oc in outs:
                        tn = normalize_team(oc.get("name"))
                        is_home = (_team_key(tn) == _team_key(home))
                        ...
                        outcomes.append({"outcomeId":oid,"outcomeName":tn.lower(),
                                         "players":{"0":{"price":oc.get("price")}}})
                    oid_counter += 2
                    markets[str(2000+mi)] = {"marketName":"Handicap (incl. extra innings)","handicap":float(handicap),"outcomes":outcomes}
                elif key == "totals":
                    line = over_p = under_p = None
                    for oc in outs:
                        nm = (oc.get("name") or "").lower()
                        if nm == "over": line=oc.get("point"); over_p=oc.get("price")
                        elif nm == "under": under_p=oc.get("price")
                    if line is None or over_p is None: continue
                    outcomes = [
                        {"outcomeId":oid_counter,"outcomeName":"over","players":{"0":{"price":over_p}}},
                        {"outcomeId":oid_counter+1,"outcomeName":"under","players":{"0":{"price":under_p}}},
                    ]
                    oid_counter += 2
                    markets[str(3000+mi)] = {"marketName":"Over Under (incl. extra innings)","handicap":float(line),"outcomes":outcomes}
...
def fetch_schedule_games(day):
    ...
    ws = day.replace(hour=15,minute=0); we = (day+timedelta(days=2)).replace(hour=10,minute=0)
    seen=set(); out=[]
    for g in games:
...
def main():
    ...
    payload={"date":now.strftime("%Y-%m-%d"),"generated_at_utc":now.strftime("%Y-%m-%dT%H:%M:%SZ"),
             "source":{"provider":source_provider,"bookmaker":"pinnacle"},
