...
def odds_get(endpoint: str, params: dict, max_retries: int = 8) -> dict | list:
    """Call OddsPapi with automatic exponential-backoff retry on 429.

    OddsPapi enforces per-minute (and possibly per-5-minute) rate limits.
    We back off aggressively (capped at 60s) and retry up to 8 times so a single
    rate-limit window (60-120s) can clear without failing the whole build.
    """
    ...
    for attempt in range(max_retries + 1):
        ...
        if response.status_code == 429:
            ...
            try:
                ...
                wait = None
            if wait is None:
                # Exponential: 2, 4, 8, 15, 25, 40, 60, 60 (capped at 60s)
                waits = [2.0, 4.0, 8.0, 15.0, 25.0, 40.0, 60.0, 60.0, 60.0]
            ...
            if attempt < max_retries:
                ...
                _t.sleep(wait)
                continue
            raise ApiError(f"429 Too Many Requests (gave up after {max_retries+1} tries over ~2min)") from None
        try:
            response.raise_for_status()
...
def main() -> None:
    ...
    # Startup cool-down: odds_get retries on 429, but give the rate-limit window a couple seconds to clear
    # in case this workflow was triggered immediately after a failed run.
    import time as _time
    print("[slate] Waiting 5s before first OddsPapi call to let rate limits settle...")
    _time.sleep(5)
    ...
    for i, fixture in enumerate(fixtures):
        ...
        if i > 0:
            _time.sleep(1.5)  # 1.5s spacing between per-fixture odds calls; total ~25s for 15 games
        try:
