...
def match_schedule_game(away_team: str, home_team: str, start_time: str | None, schedule_games: list[dict]) -> dict:
    ...
    def _swap(sched: dict) -> dict:
        """Return a copy of sched with home/away reversed to match the OddsPapi perspective."""
        out = dict(sched)
...
def build_game(
    ...
    p1_name = normalize_team(fixture.get("participant1Name") or "Team 1")
    p2_name = normalize_team(fixture.get("participant2Name") or "Team 2")
    sched = match_schedule_game(p1_name, p2_name, fixture.get("startTime"), schedule_games)
    # Use canonical away/home as determined by statsapi (OddsPapi lists p1=HOME, p2=AWAY)
...
def main() -> None:
    ...
    for i, fixture in enumerate(fixtures):
        ...
        try:
            ...
            if "429" in err_msg:
                try:
                    print(f"[slate] Rate limited on {p2_raw} @ {p1_raw}, waiting 2s and retrying...")
                    _time.sleep(2.0)
                    book = fetch_book_odds(fixture_id)
                    game = build_game(fixture, book, catalog, schedule_games, form_cache, pitcher_cache, now_utc)
                    if game:
                    ...
                    err_msg = sanitize_error(retry_exc)
            skipped.append(
                {
                    "fixtureId": fixture_id,
                    "reason": err_msg,
